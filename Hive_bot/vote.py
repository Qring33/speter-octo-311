...
def fetch_random_authors(limit=3, min_reputation=70):  
    """Fetch first 20 trending posts and pick random authors with reputation >= min_reputation"""  
    ...
    try:  
        ...
        authors = []  
        for p in posts:  
            if "author" in p and "author_reputation" in p:  
                if float(p["author_reputation"]) >= min_reputation:  
...
def main():  
    print("\nFetching random authors with reputation >= 70...\n")  
    TARGET_AUTHORS = fetch_random_authors(limit=3, min_reputation=70)  
  
    if not TARGET_AUTHORS:  
        print("No authors found with high reputation → stopping!")  
        return  
