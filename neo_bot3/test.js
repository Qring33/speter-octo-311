...
(async () => {
  // Load accounts
  const accountsDir = path.join(__dirname, "neobux_accounts_V2");
  const accountsPath = path.join(accountsDir, "neobux_accounts_V2.json");
  const invalidAccountsPath = path.join(accountsDir, "invalid_accounts.json");
  ...
  while (!loginSuccess && attempts < maxAttempts) {
    ...
    try {
      ...
      await page.goto("https://www.neobux.com/m/l/", {
      ...
      const waitForDashboard = page
        .waitForURL(url => url.href.includes("https://www.neobux.com/c/"), { timeout: 12000 })
        .then(() => "dashboard");

      const waitForCaptchaError = page
        .waitForSelector('div.f_r span.t_vermelho:has-text("The verification code is incorrect")', { timeout: 12000 })
        .then(() => "captcha_error");

      const waitForAccountError = page
        .waitForSelector('div.f_r span.t_vermelho:has-text("Error:")', { timeout: 12000 })
      ...
      if (result === "account_not_found") {
        ...
        let invalidAccounts = [];
        if (fs.existsSync(invalidAccountsPath)) {
          invalidAccounts = JSON.parse(fs.readFileSync(invalidAccountsPath, "utf8"));
        }

        invalidAccounts.push(acc);
        fs.writeFileSync(
          invalidAccountsPath,
          JSON.stringify(invalidAccounts, null, 2)
        );

        // REMOVE FROM VALID ACCOUNTS
        const updatedAccounts = accounts.filter(a => a.username !== acc.username);
        fs.writeFileSync(
          accountsPath,
