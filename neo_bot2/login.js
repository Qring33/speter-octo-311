...
(async () => {
  ...
  // ✅ FIX: reuse existing page (prevents 2 windows)
  ...
  while (!loginSuccess && attempts < maxAttempts) {
    ...
    try {
      ...
      const waitForError = page
        .waitForSelector(
          'div.f_r span.t_vermelho:has-text("The verification code is incorrect")',
          { timeout: 12000 }
        )
        .then(() => "error");

      const result = await Promise.race([waitForDashboard, waitForError]).catch(() => "timeout");
